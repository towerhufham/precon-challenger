...
export const canUseAbility = (ctx: AbilityContext): boolean => {
  ...
  return A.all(
    [
      //check category
      () => ability.category.type === "Activated",
      //check usage limits
...
export const reifyEffectUnit = (ctx: AbilityContext, eff: EffectUnit, selections: Selections): Log[] => {
  const gs = ctx.gameState
  const card = ctx.thisCard
...
export const applyAbility = (ctx: AbilityContext, selections: Selections): GameState => {
  ...
  //check triggers
  const triggers = getTriggersFromlogs(ctx.gameState, logs)
  return pipe(
    ctx.gameState,
    //apply effect units generated by the ability
    gs => applyLogs(gs, logs),
    //increment moves if activated
    gs => (ability.category.type === "Activated") ? ({...gs, moves: gs.moves+1,}) : gs,
    //add logs
...
export const getTriggersFromLog = (gs: GameState, log: Log): [number, Ability][] => {
  ...
  return [...pipe(
    ...
    A.filter(ability => ability.category.type === "Triggered"
...
export const getTriggersFromlogs = (gs: GameState, logs: Log[]): [number, Ability][] => {
  return [...pipe(
    logs,
    A.map(l => getTriggersFromLog(gs, l)),
    A.flat
  )]
}